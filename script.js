...
document.getElementById('dailyQuizStartBtn').addEventListener('click', () => {
  ...
  const body = document.getElementById('dailyQuizBody');
  body.style.display = '';
...
document.getElementById('dailyQuizFinishBtn').addEventListener('click', () => {
  ...
  resultEl.innerHTML = '<h3>Kuis hari ini selesai 🎉</h3><p>Skor: ' + s.correct + '/' + s.total + '</p>';
...
function loadNextPracticeQuestion(){
  ...
  document.getElementById('endlessQuestion').innerHTML = buildQuizHTML([PRACTICE_POOL[idx]], 'ep-' + Date.now());
}
...
document.getElementById('endlessNextBtn').addEventListener('click', () => {
  const scope = document.getElementById('endlessQuestion');
  const s = computeQuizScope(scope);
  /* cuma tambah XP kalau soal sebelumnya sudah dijawab (bukan di-skip) */
  if (s.answered > 0) {
    const gained = s.correct > 0 ? 10 : 2;
    const newXP = getXP() + gained;
    localStorage.setItem(XP_KEY, String(newXP));
    renderXPBadge();
    syncToLeaderboard();
    showToast((s.correct > 0 ? '✅ Benar! ' : '📝 ') + '+' + gained + ' XP');
  }
  loadNextPracticeQuestion();
});
