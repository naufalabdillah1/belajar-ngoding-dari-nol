...
function renderPremiumState(){
  const unlocked = localStorage.getItem(PREMIUM_KEY) === '1';
...
function renderLeaderboardList(entries){
  ...
  listEl.innerHTML = entries.map(e =>
    '<li class="' + (e.id === myId ? 'lb-self' : '') + '">' +
      '<span class="lb-name">' + e.name + (e.id === myId ? ' (kamu)' : '') + '</span>' +
...
/* =====================================================================
   REVISI: Diskusi sekarang halaman terpisah + pemilih bahasa/pelajaran,
   supaya diskusi antar pengunjung tidak campur aduk. Giscus (GitHub
   Discussions) mendukung pengelompokan diskusi lewat "data-term"
   (data-mapping="specific") — jadi tiap kali orang ganti pilihan
   bahasa/pelajaran di halaman ini, kita muat ulang widget Giscus dengan
   term yang berbeda, sehingga tiap bahasa/pelajaran punya "ruang
   obrolan" sendiri-sendiri dan tidak bentrok satu sama lain.
   CARA AKTIVASI (untuk pemilik web): sama seperti sebelumnya — aktifkan
   GitHub Discussions di repo, lalu isi data-repo/data-repo-id/
   data-category-id lewat https://giscus.app, tempel di GISCUS_CONFIG
   di bawah ini.
   ===================================================================== */
const GISCUS_CONFIG = {
  repo: 'GANTI-DENGAN-USERNAME/GANTI-DENGAN-NAMA-REPO',
  repoId: 'GANTI-DENGAN-REPO-ID',
  category: 'General',
...
function loadGiscusFor(term){
  const host = document.getElementById('discussionGiscusHost');
  host.innerHTML = '';
...
function initDiscussionUI(){
  ...
  select.addEventListener('change', () => {
    loadGiscusFor('diskusi-' + select.value);
  });
}
initDiscussionUI();
...
document.getElementById('discussionBackBtn').addEventListener('click', () => showView('home'));
