...
function renderChatMessages(docs){
  ...
  box.innerHTML = docs.map(m => {
    ...
    return '<div class="chat-msg' + (isSelf ? ' chat-self' : '') + '">' +
      '<div class="chat-msg-head"><span class="chat-msg-name">' + escapeHtml(m.name || 'Anonim') + (isSelf ? ' (kamu)' : '') + '</span><span>' + formatChatTime(m.ts) + '</span></div>' +
