...
/* ===== FITUR PREMIUM: konstanta ditaruh di paling atas =====
   PENTING: ini SENGAJA ditaruh di sini (bukan di dekat kode UI premium
   yang lebih di bawah) supaya tersedia lebih dulu untuk fungsi lain
   seperti buildExamStages() yang butuh tahu status premium sejak awal
   skrip dijalankan. Kalau dipindah ke bawah lagi, situs akan error total
   ("Cannot access before initialization") dan SEMUA tombol jadi tidak
   merespon meski terlihat bisa diklik. */
...
function typeLoop(){
  /* Kalau Beranda sedang tidak ditampilkan (user lagi buka Kuis/Ujian/dll),
     jeda animasi ini dulu — tidak ada gunanya mengubah teks yang tidak
     kelihatan, dan ini juga mengurangi resiko reflow/scroll aneh di
     background. Animasi otomatis lanjut begitu Beranda tampil lagi. */
