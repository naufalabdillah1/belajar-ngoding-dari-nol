...
function collectProgress(){
  ...
  return { checked, favorites, notes, quizAnswers, examStarted };
}
...
function restoreProgress(){
  ...
  if (data.examStarted) {
    if (examQuestionsEl.innerHTML.trim() === '') {
      examQuestionsEl.innerHTML = buildQuizHTML(EXAM_POOL, 'exam');
    }
...
document.getElementById('resetBtn').addEventListener('click', () => {
  ...
  examLiveBar.textContent = 'Terjawab: 0/' + EXAM_POOL.length + ' • Skor sementara: 0/0';
...
/* ---------- ujian akhir: kumpulkan semua soal dari semua level ---------- */
const EXAM_POOL = LEVELS.reduce((acc, lv) => acc.concat(lv.quiz), []);
...
examFinishBtn.addEventListener('click', () => {
  ...
  let noteUnanswered = unanswered > 0
    ? ('Kamu belum menjawab ' + unanswered + ' dari ' + s.total + ' soal — soal yang belum dijawab dihitung belum tepat dalam skor ini.')
    : 'Semua soal sudah dijawab.';

  examResultEl.className = 'exam-result show ' + (passed ? 'pass' : 'fail');
  examResultEl.innerHTML =
    '<h3>' + (passed ? '🎉 Lulus ujian!' : '📘 Masih perlu latihan lagi') + '</h3>' +
    '<p>Skor akhir: <b>' + s.correct + '/' + s.total + '</b> (' + pct + '%). ' + noteUnanswered + '</p>' +
    '<p>' + (passed
      ? 'Dasar-dasarmu sudah cukup kuat. Boleh lanjut ke level berikutnya, atau ulangi lagi soal yang tadi salah supaya makin melekat.'
